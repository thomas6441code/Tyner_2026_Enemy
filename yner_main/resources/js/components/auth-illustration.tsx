...
export function AuthIllustration() {
    return (
        <div className="relative w-full max-w-sm">
            ...
            <div className="relative rounded-2xl bg-white p-5 shadow-xl">
                ...
                <div className="mt-6 flex h-24 items-end justify-between gap-2">
                    {bars.map((h, i) => (
                        <div
                            key={i}
                            className={`w-full rounded-md ${i === 3 ? 'bg-primary' : 'bg-slate-100'}`}
                            style={{ height: `${h}%` }}
                        />
                    ))}
                </div>
            ...
            {/* Floating pills */}
            <div className="absolute -right-5 top-10 flex items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-medium text-slate-700 shadow-lg">
                <span className="flex h-4 w-4 items-center justify-center rounded-full bg-primary text-white">
                    <Check className="h-2.5 w-2.5" />
                </span>
                Checked in
            </div>
            <div className="absolute -left-5 bottom-12 rounded-xl bg-white px-3 py-2 text-xs font-medium text-slate-700 shadow-lg">
                On time · 08:02
            </div>
        </div>
    );
